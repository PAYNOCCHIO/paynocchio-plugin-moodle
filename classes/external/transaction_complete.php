...
class transaction_complete extends external_api {
    ...
    public static function execute(string $component, string $paymentarea, int $itemid, string $orderid): array {
        ...
        if ($orderdetails) {
            if ($orderdetails['status'] == paynocchio_helper::ORDER_STATUS_APPROVED &&
                    $orderdetails['intent'] == paynocchio_helper::ORDER_INTENT_CAPTURE) {
                $item = $orderdetails['purchase_units'][0];
                if ($item['amount']['value'] == $amount && $item['amount']['currency_code'] == $currency) {
                    $capture = $paynocchiohelper->capture_order($orderid);
                    if ($capture && $capture['status'] == paynocchio_helper::CAPTURE_STATUS_COMPLETED) {
                        $success = true;
                        ...
                        try {
                            $paymentid = payment_helper::save_payment($payable->get_account_id(), $component, $paymentarea,
                                $itemid, (int) $USER->id, $amount, $currency, 'paynocchio');

                            // Store Paynocchio extra information.
                            $record = new \stdClass();
                            $record->paymentid = $paymentid;
                            $record->pp_orderid = $orderid;

                            $DB->insert_record('paygw_paynocchio', $record);

                            payment_helper::deliver_order($component, $paymentarea, $itemid, $paymentid, (int) $USER->id);
                        } catch (\Exception $e) {
                            debugging('Exception while trying to process payment: ' . $e->getMessage(), DEBUG_DEVELOPER);
                            $success = false;
                            $message = get_string('internalerror', 'paygw_paynocchio');
                        }
                    } else {
                        $success = false;
                        $message = get_string('paymentnotcleared', 'paygw_paynocchio');
                    }
                } else {
                    $success = false;
                    $message = get_string('amountmismatch', 'paygw_paynocchio');
                }
            } else {
                $success = false;
                $message = get_string('paymentnotcleared', 'paygw_paynocchio');
            ...
            $success = false;
            $message = get_string('cannotfetchorderdatails', 'paygw_paynocchio');
        }
